...
aoi <- sf::read_sf("https://github.com/STBrinkmann/CGEI-Vancouver/raw/refs/heads/main/data/AOI.gpkg")
water <- sf::read_sf("https://github.com/STBrinkmann/CGEI-Vancouver/raw/refs/heads/main/data/water.gpkg")

tm_shape(aoi) +   tm_polygons() +
  tm_shape(water) + tm_polygons(alpha = 0.3, col = "blue") +
...
```{r echo=FALSE}
tm_shape(aoi) + tm_polygons() +
  tm_shape(sf_sample) + tm_dots(alpha = 0.5, col = "blue", size = .02) +
  tm_layout(bg.color = "transparent")
```
...
p1 <- CGEI::viewshed_list(observer = st_observer,
                          dsm_rast = dsm, dtm_rast = dtm,
                          max_distance = 250, observer_height = 1.7) %>%
  purrr::pluck(1) %>% 
  tm_shape() +
  tm_raster(palette = c("#FEDD00", "#009639"), col.legend = tm_legend_hide()) + 
  tm_layout(bg.color = "transparent", frame = FALSE) +
  tm_title(text = "1m", position = c(0.45, 1))+
  tm_legend(show=FALSE)

p2 <- CGEI::viewshed_list(observer = st_observer,
                          dsm_rast = aggregate(dsm, 10), 
                          dtm_rast = aggregate(dtm, 10),
                          max_distance = 250, observer_height = 1.7) %>%
  purrr::pluck(1) %>% 
  tm_shape() +
  tm_raster(palette = c("#FEDD00", "#009639"), col.legend = tm_legend_hide()) + 
  tm_layout(bg.color = "transparent", frame = FALSE) +
  tm_title(text = "10m", position = c(0.45, 1)) +
  tm_legend(show=FALSE)
...
library(parallel)
similarity_tbl <- mclapply(1:500, FUN = function(x) {
  compare_resolution(observer = sf_sample[x, ],
                     dsm_path = terra::sources(dsm),
                     dtm_path = terra::sources(dtm))
  }, mc.cores = 20, mc.preschedule = TRUE) %>%
  bind_rows()
```

```{r include=FALSE, warning=FALSE, message=FALSE}
similarity_tbl <- readr::read_csv("out2.csv")
```
